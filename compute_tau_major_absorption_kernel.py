...
rrtmgp_path = '/home/bart/meteo/models/rte-rrtmgp-cpp'
...
# Dummy data
#
gpoint_flavor = np.zeros(512, dtype=type_int)   # = 2 x ngpt?
band_lims_gpt = np.zeros(32, dtype=type_int)    # = 2 x nband?
jeta = np.zeros(576000, dtype=type_int)         # = ??
jtemp = np.zeros(28800, dtype=type_int)         # = ncol x nlay?
jpress = np.zeros(28800, dtype=type_int)        # = ncol x nlay?
...
tune_params = dict()
tune_params["block_size_x"] = [4] #[4,8,16,32]
tune_params["block_size_y"] = [4] #[4,8,16,32]
tune_params["block_size_z"] = [4] #[4,8,16,32]
